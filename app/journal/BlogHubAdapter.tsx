...
const BlogHubAdapter: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brand-cream text-brand-grey-green flex items-center justify-center">
    ...
    return (
      <div className="min-h-screen bg-brand-cream text-brand-grey-green flex items-center justify-center">
        <div className="text-center">
          ...
          <button 
            onClick={fetchData}
            className="mt-4 px-4 py-2 bg-brand-green text-white hover:bg-brand-green/90 transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-brand-cream text-brand-grey-green">
      {/* Navigation */}
      ...
      {/* Page Header */}
      <div className="bg-white border-b border-brand-green/20 pt-32">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <ChevronRight size={16} className="text-brand-grey-green/40" />
            <span className="text-brand-grey-green/70">Journal</span>
          </div>
          
          <h1 className="text-4xl font-bold text-center mb-4 text-brand-gold">BEHIND THE DESIGNS</h1>
      ...
      <div className="bg-white border-b border-brand-green/20 md:sticky md:top-0 md:z-10">
        <div className="container mx-auto px-4">
          {/* Mobile: vertical accordion */}
          <div className="md:hidden py-4 space-y-4">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-brand-grey-green/60" size={16} />
              <input
                type="text"
                ...
                className="w-full pl-10 pr-4 py-2 border-2 border-brand-green/30 rounded-lg bg-brand-light-green text-brand-grey-green focus:outline-none focus:border-brand-green disabled:opacity-50 disabled:cursor-not-allowed"
              />
            </div>
              ...
              <button
                ...
                className={`w-full text-left text-sm font-semibold uppercase tracking-wide transition-colors flex items-center justify-between gap-2 ${
                  !selectedParent && !selectedCategory 
                    ? 'text-brand-gold'
                    : 'text-brand-grey-green hover:text-brand-gold'
              ...
              {parentCategories.map((parent) => (
                <div key={parent.id}>
                  ...
                  >
                    ...
                    <span className="text-xs text-brand-grey-green/40">{mobileOpenParentId === parent.id ? '−' : '+'}</span>
                  </button>
                    ...
                    <div className="mt-2 pl-4 space-y-1">
                      {parent.children.map((child) => (
                        <button
                          ...
                          disabled={categoryLoading}
                          className={`w-full text-left px-2 py-1 text-sm text-brand-grey-green hover:text-brand-gold hover:bg-brand-light-green ${
                            categoryLoading ? 'opacity-50 cursor-not-allowed' : ''
                          }`}
                        >
                          {child.name} <span className="text-brand-grey-green/60">({child.count})</span>
                        </button>
                      ))}
                    </div>
                  )}
          ...
          <div className="hidden md:flex items-center justify-between py-4">
              ...
              {parentCategories.map((parent) => (
                <div key={parent.id} className="relative group">
                  ...
                  {parent.children.length > 0 && (
                    <div className="absolute top-full left-0 mt-2 w-64 bg-white border-2 border-brand-green/20 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20">
                      <div className="py-2">
                        {parent.children.map((child) => (
                          <button
                            key={child.id}
                            onClick={() => handleCategorySelect(child)}
                            disabled={categoryLoading}
                            className={`w-full text-left px-4 py-2 text-sm text-brand-grey-green hover:bg-brand-light-green hover:text-brand-gold transition-colors flex items-center justify-between ${
                              categoryLoading ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                          >
                            <span>{child.name}</span>
                            <div className="flex items-center gap-2">
                              <span className="text-brand-grey-green/60">({child.count})</span>
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                disabled={categoryLoading}
                className="pl-10 pr-4 py-2 border-2 border-brand-green/30 rounded-lg bg-brand-light-green text-brand-grey-green focus:outline-none focus:border-brand-green disabled:opacity-50 disabled:cursor-not-allowed"
              />
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {(selectedParent || selectedCategory) && (
          <div className="mb-6">
            <button 
              ...
              disabled={categoryLoading}
              className="flex items-center gap-2 text-brand-gold hover:text-brand-green transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <ArrowLeft size={20} />
              Back to All Posts
            </button>
        ...
        {!categoryLoading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post) => {
              return (
                <article key={post.id} className="bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 rounded-lg border-2 border-brand-green/20">
                  {/* Featured Image */}
                  ...
                  <div className="p-6">
                    ...
                    {post.categories && post.categories.length > 0 && post.categories[0] && (
                      <div className="mb-3">
                        <span className="inline-block px-3 py-1 bg-brand-light-green text-brand-grey-green text-xs font-semibold uppercase tracking-wide rounded">
                          {post.categories[0]}
                        </span>
                      </div>
                    )}
                    ...
                    {/* Title */}
                    <h2 className="text-xl font-bold mb-3 line-clamp-2 hover:text-brand-gold transition-colors text-brand-grey-green">
                      <Link href={`/journal/${post.slug}`}>
                        {post.title}
                      </Link>
                    </h2>
                    ...
                    {/* Excerpt */}
                    <p className="text-brand-green text-sm mb-4 line-clamp-3">
                      {post.excerpt || 'No excerpt available'}
                    </p>
                    
                    {/* Meta */}
                    <div className="flex items-center justify-between text-xs text-brand-grey-green/70">
        ...
        {!categoryLoading && filteredPosts.length === 0 && (
          <div className="text-center py-12">
            <div className="text-brand-grey-green/40 mb-4">
              <svg className="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <h3 className="text-lg font-semibold mb-2 text-brand-grey-green">No Posts Found</h3>
            <p className="text-brand-grey-green/70">
